(function attachBoxeurProgression(root, factory) {
  ...
  const STATE_KIND = "boxeur-progression";
  const LEGACY_STATE_KINDS = Object.freeze(["boxeur-v2-progression"]);
  ...
  function isProgressionState(value) {
    return Boolean(value && typeof value === "object"
      && (value.kind === STATE_KIND || LEGACY_STATE_KINDS.includes(value.kind))
  ...
  function findExplicitProgress(source) {
    ...
    if (isProgressionState(source) && source.progress) return source.progress;
    if (isProgressionState(source.progression)) return source.progression.progress;
    if (isProgressionState(source.progressionState)) {
  ...
  function migrate(source, options = {}) {
    ...
    const already = isProgressionState(source)
      ? source
      : isProgressionState(source.progressionState)
        ? source.progressionState
        : isProgressionState(source.progression)
          ? source.progression
          : null;
    if (already) {
      return {
        state: createState(already, { config }),
        report: { source: "progression", ignoredLegacyProgress: false, reserveImported: emptyStats(0) },
      };
    }
