(function attachBoxeurWorld(root, factory) {
  ...
  function sparringPartner(career = {}) {
    const supplied = career.careerSparringPartner && typeof career.careerSparringPartner === "object"
      ? career.careerSparringPartner
      : {};
  ...
  function preparation(career) {
    if (career.careerPreparation && typeof career.careerPreparation === "object") {
      const supplied = career.careerPreparation;
      return {
  ...
  function onboardingObjective(career) {
    const step = career.careerOnboardingStep;
    const onboarding = career.careerOnboarding;
    ...
    const partner = step.sparringPartner && typeof step.sparringPartner === "object"
      ? sparringPartner({ careerSparringPartner: step.sparringPartner })
  ...
  function guideAction(currentObjective, currentLocationId) {
    if (currentObjective.actionMode === "review-and-confirm") {
      return `<div class="career-onboarding-actions"><button class="primary-button" type="button" data-career-week-handoff>Confirmer semaine</button></div>`;
    }
    if (currentObjective.actionMode === "quick-plan") {
      return `<button class="primary-button" type="button" data-career-week-quick>Suivre le plan rapide</button>`;
    }
    if (currentLocationId !== currentObjective.locationId) {
      return currentObjective.locationId === "map"
        ? ""
        : `<button class="primary-button" type="button" data-career-location="${escapeHTML(currentObjective.locationId)}">M’y rendre</button>`;
    }
    if (currentObjective.type === "membership" && currentLocationId === "boxing-gym") {
      return `<button class="primary-button" type="button" data-career-gym-zone="reception">Aller à l’accueil</button>`;
    }
    if (currentObjective.type === "membership-renewal" && currentLocationId === "boxing-gym") {
      return `<button class="primary-button" type="button" data-career-gym-zone="reception">Renouveler à l’accueil</button>`;
    }
    if (currentObjective.type === "work-priority" && currentLocationId === "work") {
      return `<button class="primary-button" type="button" data-career-toggle-work aria-pressed="true">Ne pas travailler cette semaine</button>`;
    }
    if (currentObjective.type === "home-training" && currentLocationId === "home") {
      return `<button class="primary-button" type="button" data-career-home-menu="training">Ouvrir le menu du sous-sol</button>`;
    }
    if (currentObjective.type === "roadwork" && currentLocationId === "home") {
      return `<button class="primary-button" type="button" data-career-home-menu="running">Ouvrir le menu Course</button>`;
    }
    if (currentObjective.type === "recreational-course" && currentLocationId === "boxing-gym") {
      return `<button class="primary-button" type="button" data-career-gym-zone="coach">Voir le cours récréatif</button>`;
    }
    if (currentObjective.type === "objective" && currentLocationId === "boxing-gym") {
      return `<button class="primary-button" type="button" data-career-gym-zone="coach">Voir le cours récréatif</button>`;
    }
    if (currentObjective.type === "recovery" && currentLocationId === "home") {
      return `<button class="primary-button" type="button" data-career-home-action="rest">Ajouter une journée de repos</button>`;
    }
    if (currentObjective.type === "job" && currentLocationId === "work") {
      return `<button class="primary-button" type="button" data-career-open-job-menu>Choisir mon emploi</button>`;
    }
    if (currentObjective.type === "sparring" && currentLocationId === "boxing-gym") {
      return `<button class="primary-button" type="button" data-career-remy-sparring>Faire le sparring pédagogique</button>`;
    }
    return "";
  }
  ...
  function renderObjectiveCard(currentObjective, currentLocationId = "map") {
    ...
    if (!currentObjective.onboarding) {
      return `<section class="career-objective-card"><p class="eyebrow">Prochaine étape</p><h3>${escapeHTML(currentObjective.title)}</h3><p>${escapeHTML(currentObjective.detail)}</p>${destination}</section>`;
    }
    ...
    const locationClass = currentLocationId === "map" ? "" : " career-location-guide";
    return `<section class="career-objective-card career-onboarding-card ${requirementClass}${locationClass}" data-career-onboarding-step="${escapeHTML(currentObjective.id)}">
      <div class="career-objective-heading"><p class="eyebrow">Guide récréatif</p><span class="career-objective-requirement ${requirementClass}">${requirement}</span></div>
      <div class="career-onboarding-track" aria-label="Parcours guidé : semaine ${currentWeek} sur ${currentObjective.remyWeek} avant le sparring de ${escapeHTML(partnerName)}">
      ...
      <h3>${escapeHTML(currentObjective.title)}</h3><p>${escapeHTML(currentObjective.detail)}</p><p class="career-guide-instruction"><strong>Comment faire :</strong> ${escapeHTML(instruction)}</p>${actions}
    </section>`;
  }
  ...
  function appointmentList(career) {
    const candidates = [career.careerAppointments, career.appointments, career.timeState?.appointments];
    return candidates.find(Array.isArray) || [];
  }

  function nextScheduledAppointment(career) {
    const currentSlot = Number(career.careerClock?.absoluteSlot);
  ...
  function developerTestBanner(career) {
    const testState = career.careerDeveloperTest;
    ...
    return `<aside class="career-test-mode-banner" aria-label="Mode test actif">
      ...
      <button type="button" data-career-restore-career${returnDisabled}>Retour à ma carrière</button>
    </aside>`;
  }

  function workManagement(career) {
    const job = career.careerJob && typeof career.careerJob === "object" ? career.careerJob : null;
    ...
    const applicationJob = application ? career.careerJobApplicationLabel || "Emploi visé" : "";
    if (job) {
      const planned = career.careerWorkPlan?.planned !== false;
      const missed = Math.max(0, Math.min(2, Math.round(Number(career.missedWorkWeeks) || 0)));
      const workStatus = planned
        ? `Le travail est prévu cette semaine et réserve ${Math.max(0, Math.round(Number(career.careerWorkPlan?.cost) || 0))} énergie. Tu peux le retirer avant la confirmation.`
      ...
      const attendance = missed > 0
        ? `<p class="career-work-attendance-warning"><strong>${missed}/3 absence${missed > 1 ? "s" : ""} consécutive${missed > 1 ? "s" : ""}.</strong> La troisième entraîne le congédiement.</p>`
        : "";
      const applicationCopy = application
        ...
        : "";
      const shiftButton = `<button class="${planned ? "secondary-button" : "primary-button"}" type="button" data-career-toggle-work aria-pressed="${planned}">${planned ? "Retirer le travail de ma semaine" : "Ajouter le travail à ma semaine"}</button>`;
      return `<section class="career-work-management" aria-labelledby="career-work-management-title">
        <div><p class="eyebrow">Emploi actuel</p><h3 id="career-work-management-title">${escapeHTML(job.title || "Emploi actif")}</h3></div>
        ...
        <div class="career-work-management-actions">${shiftButton}<button class="secondary-button" type="button" data-career-open-job-menu>Voir mon emploi</button></div>
      </section>`;
    }
    if (application) {
      ...
      return `<section class="career-work-management" aria-labelledby="career-work-management-title">
        <div><p class="eyebrow">Candidature en cours</p><h3 id="career-work-management-title">${escapeHTML(applicationJob)}</h3></div>
        ...
        <div class="career-work-management-actions"><button class="secondary-button" type="button" data-career-open-job-menu>Voir ou changer la candidature</button></div>
      </section>`;
    }
    return `<section class="career-work-management ${firstJobRequired ? "required" : ""}" aria-labelledby="career-work-management-title">
      <div><p class="eyebrow">${firstJobRequired ? "Étape obligatoire" : "Revenu facultatif"}</p><h3 id="career-work-management-title">${firstJobRequired ? "Choisis ton premier emploi" : "Chercher un emploi"}</h3></div>
      ...
      <button class="primary-button" type="button" data-career-open-job-menu>${firstJobRequired ? "Choisir mon emploi" : "Voir les emplois"}</button>
    </section>`;
  }
  ...
  function renderNavigation(activeView = "map") {
    ...
    const buttons = PRIMARY_NAVIGATION.map(item => {
      ...
      return `<button${selected ? ' class="active" aria-current="page"' : ""} type="button" data-career-nav="${item.id}">${item.label}</button>`;
    }).join("");
  ...
  function render(career) {
    ...
    return `<div class="career-world-layout">
      ...
      <div class="career-side-stack">
        <header class="career-world-bar">
          <div class="career-now-time"><span>Semaine</span><strong>${String(career.week || 1).padStart(2, "0")}</strong><small><span>${escapeHTML(dateLabel)}</span><b class="career-now-money" aria-label="Argent disponible ${escapeHTML(moneyLabel)}">${escapeHTML(moneyLabel)}</b></small></div>
        </header>
        <aside class="career-now-panel" aria-label="Situation actuelle">
          ${currentObjective ? renderObjectiveCard(currentObjective, "map") : ""}
          <section class="career-readiness-card ${prep.tone}"><span>État de préparation</span><strong>${escapeHTML(prep.label)}</strong><p>${escapeHTML(prep.detail)}</p><div class="career-vitals"><span>Énergie <b>${Math.round(career.energy || 0)} %</b></span><span>Fatigue <b>${Math.round(career.fatigue || 0)} %</b></span></div></section>
          <section class="career-appointment-card"><span>Prochain rendez-vous</span><strong>${escapeHTML(nextAppointment(career))}</strong><button type="button" data-career-open-calendar>Voir les sept prochains jours</button></section>
        </aside>
      </div>
    </div>
    <section class="career-location-sheet" role="dialog" aria-modal="true" aria-label="Lieu du quartier" tabindex="-1" hidden></section>`;
  }
  ...
  function renderLocation(locationId, career) {
    ...
    const actions = location.id === "arena" ? `<button class="primary-button" type="button" data-career-open-calendar>Ouvrir le calendrier</button>` : "";
