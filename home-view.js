(function attachBoxeurHomeView(root, factory) {
  ...
  function renderHotspot(zone, context) {
    ...
    const reasonId = `career-home-zone-${zone.id}-reason`;
    ...
    const planned = state.planned ? ' data-career-home-planned="true"' : "";
    const reason = state.available ? "" : `<span class="career-home-hotspot-reason" id="${reasonId}">${escapeHTML(state.reason)}</span>`;
    const detail = state.planned ? "Planifié pour cette semaine" : zone.detail;
    const target = zone.menu
      ? `data-career-home-menu="${escapeHTML(zone.menu)}"`
      : `data-career-home-action="${escapeHTML(zone.action)}"`;

    if (zone.id === "kitchen") {
      return `<div class="career-home-fridge-prototype">
        <button type="button" class="career-home-fridge-button" data-career-home-zone="${zone.id}" ${target}${planned}${pressed}${disabledAttributes} aria-label="${escapeHTML(zone.label)}. ${escapeHTML(detail)}.">
          <span class="career-home-fridge-frame" aria-hidden="true"><img class="career-home-fridge-image career-home-fridge-image-desktop" src="assets/maison-v2-desktop.jpg" alt=""><img class="career-home-fridge-image career-home-fridge-image-mobile" src="assets/maison-v2-mobile.jpg" alt=""></span>
          <span class="career-home-fridge-title" aria-hidden="true">Cuisine</span>
        </button>${reason}
      </div>`;
    }

    return `<div class="career-home-hotspot-wrap career-home-hotspot-${zone.id}${state.planned ? " planned" : ""}">
      <button type="button" class="career-home-hotspot" data-career-home-zone="${zone.id}" ${target}${planned}${pressed}${disabledAttributes} aria-label="${escapeHTML(zone.label)}. ${escapeHTML(detail)}.">
  ...
  function renderAction(action, context) {
    ...
    const helpId = `career-home-action-${action.id}-help`;
    ...
    return `<div class="career-home-action${state.available ? "" : " unavailable"}${state.planned ? " planned" : ""}">
      <button type="button" data-career-home-action="${action.id}" aria-label="${escapeHTML(accessibleLabel)}" aria-describedby="${helpId}"${planned}${pressed}${disabledAttributes}>
        <span class="career-home-action-title">${escapeHTML(action.label)}</span>
        <span class="career-home-action-meta"><b>${escapeHTML(state.kindLabel)}</b><em>${escapeHTML(state.impact)}</em></span>
        <span class="career-home-action-command">${escapeHTML(command)}</span>
      </button>
  ...
  function renderActionGroup(group, context) {
    ...
    return `<section class="career-home-action-group career-home-action-group-${group.id}" aria-labelledby="career-home-action-group-${group.id}">
      <div class="career-home-action-group-heading"><h4 id="career-home-action-group-${group.id}">${escapeHTML(group.label)}</h4><p>${escapeHTML(group.detail)}</p></div>
      <div class="career-home-action-grid">${matching.map(action => renderAction(action, context)).join("")}</div>
    </section>`;
  }
  ...
  function renderMenu(menuId, rawContext) {
    ...
    if (!menu) return "";
    return `<section class="career-home-menu" aria-labelledby="career-home-menu-title">
      <header><div><p class="eyebrow">Maison · semaine ${context.clock.week}</p><h2 id="career-home-menu-title">${escapeHTML(menu.label)}</h2></div><button type="button" class="secondary-button" data-career-home-menu-close>Retour à la maison</button></header>
      <p>${escapeHTML(menu.detail)}</p>
      <div class="career-home-menu-actions">${renderActionGroup(menu, context)}</div>
    </section>`;
  }
  ...
  function renderWeekPlan(context) {
    ...
    return `<section class="career-home-week-plan career-place-week-plan${capacity.full ? " full" : ""}" aria-labelledby="career-home-week-plan-title" aria-live="polite">
      <div class="career-home-week-plan-heading career-place-week-plan-heading"><div><p class="eyebrow">Planification</p><h3 id="career-home-week-plan-title">${capacity.full ? "Programme complet" : escapeHTML(context.plan.title)}</h3></div><strong>${capacity.remaining} / ${capacity.allowed}</strong></div>
  ...
  function render(rawContext) {
    ...
    return `<div class="career-home-view career-place-view">
      <header class="career-home-header career-place-header">
        <div><p class="eyebrow">Maison</p><h2>Chez toi, ${escapeHTML(context.profile.firstName)}</h2><p class="career-place-meta">${escapeHTML(context.careerStatusLabel)} · Semaine ${context.clock.week} · ${escapeHTML(context.clock.dayLabel)} · ${escapeHTML(context.clock.dateLabel)}</p></div>
        <button type="button" class="secondary-button" data-career-leave-home>Retour à la carte</button>
      </header>
      <div class="career-home-layout career-place-layout">
        <section class="career-home-scene career-place-scene" aria-labelledby="career-home-scene-title">
          <h3 id="career-home-scene-title" class="sr-only">Pièces interactives de la maison</h3>
          <picture>
          ...
          <div class="career-home-hotspots">${hotspots}</div>
        </section>
        <aside class="career-home-dashboard career-place-dashboard" aria-label="Programme et récupération">
          ${renderWeekPlan(context)}
          <section class="career-home-condition career-place-condition" aria-label="État de récupération actuel">
          ...
          <section class="career-home-recommendation career-place-card ${context.condition.recommendationTone}" aria-labelledby="career-home-recommendation-title">
            <p class="eyebrow">Conseil avant de planifier</p><h3 id="career-home-recommendation-title">${escapeHTML(context.condition.recommendation)}</h3>
  ...
  function renderResult(rawResult) {
    ...
    const recommendation = result.recommendation
      ? `<p class="career-home-result-recommendation"><strong>À retenir :</strong> ${escapeHTML(safeText(result.recommendation, "", 300))}</p>`
      : "";

    return `<section class="career-home-result" aria-labelledby="career-home-result-title" aria-live="polite">
      <p class="eyebrow">Programme de la maison</p><h2 id="career-home-result-title">${escapeHTML(title)}</h2>
      <p>${escapeHTML(summary)}</p><p class="career-home-result-time">${escapeHTML(timeLabel)}</p>
      <ul class="career-home-result-changes" aria-label="Changements au programme">${changeMarkup}</ul>
      ${recommendation}
      <div class="career-home-result-actions"><button type="button" class="secondary-button" data-career-home-result-close>Continuer à la maison</button><button type="button" class="primary-button" data-career-leave-home>Retour à la carte</button></div>
    </section>`;
  }
