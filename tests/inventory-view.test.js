...
const inventoryView = require("../inventory-view.js");
...
test("rend les suppléments hors de la fiche Boxeur avec des repères explicites", () => {
  ...
  assert.match(html, /<section class="career-inventory-view" aria-labelledby="career-inventory-title">/);
  assert.match(html, /<h2 id="career-inventory-title">Ton inventaire<\/h2>/);
  assert.doesNotMatch(html, /Fiche du boxeur/i);
  assert.doesNotMatch(html, /data-career-close-inventory|Retour à la carte/);
  assert.match(html, /data-career-inventory-category="supplements"/);
  assert.match(html, /<ul class="career-inventory-grid" role="list">/);
...
test("reste extensible aux catégories et actions d’objets futures", () => {
  ...
  assert.match(html, /data-career-inventory-category="souvenirs"/);
  assert.match(html, /data-career-inventory-category="equipment"/);
  assert.match(html, /<button type="button" data-career-inventory-action="equip" data-career-inventory-item="corde">Mettre dans le sac<\/button>/);
});
...
test("rend les blocages et les actions indisponibles compréhensibles au lecteur d’écran", () => {
  ...
  assert.match(html, /class="career-inventory-access" role="status"/);
  assert.match(html, /Sac remisé/);
  assert.match(html, /Reviens après le tutoriel\./);
  assert.match(html, /data-career-inventory-action="use"[^>]*disabled aria-disabled="true" aria-describedby="career-inventory-item-gants-1-reason"/);
  assert.match(html, /id="career-inventory-item-gants-1-reason">L’inventaire n’est pas accessible pour le moment\.<\/small>/);
...
test("fournit un état vide et échappe les données avant de les insérer dans le HTML", () => {
  ...
  assert.match(html, /class="career-inventory-empty" role="status"/);
...
test("la feuille de style garantit une vue mobile défilable et des cibles accessibles", () => {
  ...
  assert.match(css, /\.career-inventory-view\s*\{[^}]*width:\s*min\(1060px, 100%\)/s);
  assert.match(css, /\.career-inventory-view\s*\{[^}]*overflow-x:\s*hidden[^}]*overflow-y:\s*auto/s);
  assert.match(css, /\.career-inventory-item-action button\s*\{[^}]*min-height:\s*44px/s);
  assert.match(css, /\.career-inventory-view button:focus-visible[^}]*outline:/s);
  assert.match(css, /@media \(max-width: 700px\)[\s\S]*?\.career-inventory-view\s*\{[^}]*width:\s*100%[^}]*height:\s*100dvh/s);
  assert.match(css, /@media \(max-width: 700px\)[\s\S]*?\.career-inventory-grid\s*\{[^}]*grid-template-columns:\s*minmax\(0, 1fr\)/s);
