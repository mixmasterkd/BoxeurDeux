...
const work = require("../work-view.js");

function activeContext(overrides = {}) {
  return {
    profile: { firstName: "Alex" },
    week: 3,
    careerClock: { week: 3, dayLabel: "Mercredi · soir" },
    careerDateLabel: "18 septembre 2026",
    careerJob: {
      id: "office",
...
test("rend un environnement adapté à l’emploi actif avec les zones de travail", () => {
  ...
  assert.match(html, /class="career-work-view career-place-view career-work-view-office"/);
  assert.match(html, /assets\/emploi-bureau-v2-desktop\.png/);
  assert.match(html, /assets\/emploi-bureau-v2-mobile\.png/);
  assert.match(html, /data-career-work-zone="schedule"/);
  assert.match(html, /data-career-work-zone="job"/);
  assert.doesNotMatch(html, /data-career-work-zone="mini-game"/);
...
test("affiche le babillard quand aucun emploi n’est actif", () => {
  ...
  assert.match(html, /career-work-view-unemployed/);
  assert.match(html, /career-work-board-scene/);
  assert.match(html, /Ton premier emploi est requis/);
  assert.equal((html.match(/data-career-work-zone="employment"/g) || []).length, 4);
...
test("permet de postuler directement sur le babillard après un emploi perdu", () => {
  ...
  assert.doesNotMatch(html, /data-career-work-zone="employment"/);
...
test("garde la bascule hebdomadaire dans le menu Horaire", () => {
  const html = work.renderMenu("schedule", activeContext({ careerWorkPlan: { planned: false, cost: 30 } }));
...
test("explique et verrouille le travail quand la capacité ou la condition ne le permet plus", () => {
  ...
  assert.match(html, /data-career-toggle-work[^>]+disabled/);
...
test("échappe les données injectées dans la scène d’emploi", () => {
  const html = work.render(activeContext({
    ...
    careerJob: { id: "office", title: '<script>boom()</script>', wage: 120, schedule: '<b>danger</b>' },
  }));
