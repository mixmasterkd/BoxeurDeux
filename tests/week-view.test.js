...
const view = require("../week-view.js");
...
test("n’affiche aucune réserve automatique et présente un risque de récupération sans bloquer la confirmation", () => {
  ...
  assert.doesNotMatch(launcher, /gardés pour le repos|data-career-week-rest-reserve/);
  assert.match(plan, /Repos forcé probable/);
  assert.match(plan, /10 points pourraient être occupés/);
  assert.match(plan, /data-career-week-confirm/);
  assert.doesNotMatch(plan, /data-career-week-rest-reserve/);
});
...
test("le lanceur montre l'énergie restante et mène au bâtisseur avant la confirmation", () => {
  ...
  assert.match(html, /data-career-week-detailed/);
  assert.match(html, /Confirmer semaine/);
  assert.match(html, /data-career-week-quick/);
  assert.doesNotMatch(html, /data-career-week-confirm/);
  assert.doesNotMatch(html, /data-career-week-handoff/);
});
...
test("le plan rapide peut être bloqué et la confirmation reste dans le bâtisseur", () => {
  ...
  assert.match(launcher, /data-career-week-quick disabled aria-disabled="true"/);
  assert.match(launcher, /data-career-week-detailed/);
  assert.doesNotMatch(launcher, /data-career-week-confirm/);
  assert.match(launcher, /role="status">Choisis d’abord ton emploi\./);

  assert.match(confirmBlocked, /data-career-week-confirm disabled aria-disabled="true"/);
...
test("le plan détaillé permet de retirer les choix facultatifs mais protège les engagements fixes", () => {
  ...
  assert.match(html, /id="career-week-plan-title">Technique &amp; récupération/);
  assert.match(html, /Emploi · Lundi au vendredi/);
  assert.match(html, /Prévu par défaut/);
  assert.match(html, /Mitaines &lt;techniques&gt;/);
  assert.match(html, /data-career-week-remove="gym-&lt;jab&gt;"/);
  assert.match(html, /aria-label="Retirer Mitaines &lt;techniques&gt; du plan"/);
  assert.doesNotMatch(html, /data-career-week-remove="work-default"/);
  assert.match(html, /data-career-week-plan-close aria-label="Fermer le plan de la semaine"/);
...
test("le bilan échappe les données et annonce les résultats importants", () => {
  ...
  assert.match(html, /data-career-week-summary-close/);
});
...
test("explique le premier bilan guidé et annonce la semaine suivante", () => {
  ...
  assert.match(html, /career-week-summary-guide/);
