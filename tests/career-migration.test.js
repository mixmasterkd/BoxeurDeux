...
const migration = require("../career-migration.js");
...
test("expose une API CommonJS et globale adossée à BoxeurTime", () => {
  assert.equal(globalThis.BoxeurCareerMigration, migration);
  assert.equal(migration.CAPSULE_VERSION, 3);
  assert.equal(migration.LEGACY_TRAINING_CARRY_VERSION, 1);
  assert.equal(migration.EXPECTED_SOURCE_VERSION, 6);
  assert.equal(time.STAT_GAIN_SCALE, 0.024);
});

test("convertit les clés, champs et marqueurs historiques vers le schéma neutre", () => {
  const capsule = migration.migrateToCareerCapsule(v5Snapshot(), { migratedAt: MIGRATED_AT });
  const oldCapsule = structuredClone(capsule);
  oldCapsule.kind = "boxeur-deux-career-v2-capsule";
  oldCapsule.version = 2;
  oldCapsule.previewRuntime = {
    career: {
      v2SupplementState: { kind: "boxeur-v2-supplements", inventory: { "protein-bar": 2 } },
      v2TrainerState: { kind: "boxeur-v2-private-trainer", activeProgram: null },
      v2WeekPlannerState: { kind: "boxeur-v2-week-planner", entries: [] },
    },
    weekPlanner: { kind: "boxeur-v2-week-planner", entries: [] },
  };
  oldCapsule.timeState.history = [{ id: "v2-work:courier", activityId: "v2-remy-sparring:6" }];
  const source = v5Snapshot({
    state: {
      v2SupplementState: { kind: "boxeur-v2-supplements", inventory: { "protein-bar": 2 } },
      v2TrainerState: { kind: "boxeur-v2-private-trainer", activeProgram: null },
      v2WeekPlannerState: { kind: "boxeur-v2-week-planner", entries: [] },
      progressionState: { kind: "boxeur-v2-progression", schemaVersion: 1 },
    },
    v2PreviewCapsule: oldCapsule,
  });
  const before = structuredClone(source);

  const migrated = migration.migrateCareerSnapshot(source);

  assert.deepEqual(source, before, "la conversion doit demeurer immutable");
  assert.equal(migrated.version, 6);
  assert.equal("v2PreviewCapsule" in migrated, false);
  assert.equal(migrated.careerCapsule.kind, "boxeur-deux-career-capsule");
  assert.equal(migrated.careerCapsule.version, 3);
  assert.equal(migrated.state.supplementState.kind, "boxeur-supplements");
  assert.equal(migrated.state.trainerState.kind, "boxeur-private-trainer");
  assert.equal(migrated.state.weekPlannerState.kind, "boxeur-week-planner");
  assert.equal(migrated.state.progressionState.kind, "boxeur-progression");
  assert.equal(migrated.careerCapsule.previewRuntime.career.supplementState.kind, "boxeur-supplements");
  assert.deepEqual(migrated.careerCapsule.timeState.history[0], { id: "work:courier", activityId: "remy-sparring:6" });

  const directState = migration.migrateCareerSnapshot(source.state);
  assert.equal(directState.version, 6);
  assert.equal(directState.state.profile.firstName, "Alex");
  assert.equal(directState.state.supplementState.kind, "boxeur-supplements");
});

test("migration additive, immutable et idempotente", () => {
  const source = v5Snapshot();
  const before = structuredClone(source);
  const capsule = migration.migrateToCareerCapsule(source, { migratedAt: MIGRATED_AT, seed: "alex" });
  ...
  assert.deepEqual(capsule.legacySnapshot, migration.migrateCareerSnapshot(source));
  ...
  const secondPass = migration.migrateToCareerCapsule(capsule, { migratedAt: "2099-01-01T00:00:00Z" });
...
test("rollback restitue le snapshot migré et une copie indépendante", () => {
  const source = v5Snapshot({ customEnvelope: { checksum: "abc" } });
  const capsule = migration.migrateToCareerCapsule(source, { migratedAt: MIGRATED_AT });
  capsule.timeState.condition.energy = 1;
  capsule.legacyPendingPlan.push("not-in-source");

  const restored = migration.rollbackCareerMigration(capsule);
  assert.deepEqual(restored, migration.migrateCareerSnapshot(source));
...
test("progression partielle devient de l’XP ciblée cumulative sans hausse immédiate", () => {
  ...
  const extreme = migration.migrateToCareerCapsule(v5Snapshot({ state: {
...
test("répare rétrocompatiblement une ancienne capsule de carrière sans réserve", () => {
  ...
  const current = migration.migrateToCareerCapsule(source, { migratedAt: MIGRATED_AT });
  const olderCapsule = structuredClone(current);
  delete olderCapsule.legacyTrainingCarry;

  const repaired = migration.migrateToCareerCapsule(olderCapsule);
  assert.equal(repaired.version, 3, "le correctif met la capsule au format courant");
  ...
  assert.deepEqual(migration.migrateToCareerCapsule(repaired), repaired, "une capsule réparée redevient idempotente");
});

test("met à niveau une ancienne capsule à statistiques fractionnaires sans perdre sa progression", () => {
  const current = migration.migrateToCareerCapsule(v5Snapshot(), { migratedAt: MIGRATED_AT });
  const olderCapsule = structuredClone(current);
  ...
  const upgraded = migration.migrateToCareerCapsule(olderCapsule);
...
test("identifie les phases récréative, amateur, tournoi et professionnelle", () => {
  ...
  cases.forEach(([state, expected], index) => {
    const capsule = migration.migrateToCareerCapsule(v5Snapshot({ state }), {
      migratedAt: MIGRATED_AT,
      seed: `phase-${index}`,
    });
...
test("borne les données invalides et produit quand même un état temps sain", () => {
  ...
  const capsule = migration.migrateToCareerCapsule(source, { migratedAt: "invalide" });

  assert.equal(capsule.sourceVersion, 6);
