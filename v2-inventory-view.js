(function attachBoxeurInventoryView(root, factory) {
  ...
  function renderItem(item) {
    const titleId = `v2-inventory-item-${item.domId}`;
    const reasonId = `${titleId}-reason`;
    const kind = item.kindLabel ? `<span class="v2-inventory-item-kind">${escapeHTML(item.kindLabel)}</span>` : "";
    ...
    const compromise = item.compromise
      ...
      : "";
    const facts = benefit || compromise ? `<dl class="v2-inventory-item-facts">${benefit}${compromise}</dl>` : "";
    const note = item.note ? `<p class="v2-inventory-item-note">${escapeHTML(item.note)}</p>` : "";
    let action = "";
    if (item.action) {
      const disabled = item.action.available ? "" : ` disabled aria-disabled="true" aria-describedby="${reasonId}"`;
      const reason = item.action.available ? "" : `<small class="v2-inventory-action-reason" id="${reasonId}">${escapeHTML(item.action.disabledReason)}</small>`;
      action = `<div class="v2-inventory-item-action"><button type="button" data-v2-inventory-action="${escapeHTML(item.action.id)}" data-v2-inventory-item="${escapeHTML(item.id)}"${disabled}>${escapeHTML(item.action.label)}</button>${reason}</div>`;
    }
    return `<li class="v2-inventory-item status-${escapeHTML(item.status)}">
      <article aria-labelledby="${titleId}">
        <header><div>${kind}<h4 id="${titleId}">${escapeHTML(item.label)}</h4></div><span class="v2-inventory-quantity" aria-label="Quantité : ${item.quantity}">×${item.quantity}</span></header>
        <p>${escapeHTML(item.description)}</p>
        ${facts}${note}
        <footer><span class="v2-inventory-status"><span aria-hidden="true"></span>${escapeHTML(item.statusLabel)}</span>${action}</footer>
      </article>
    </li>`;
  }

  function renderCategory(category, items) {
    const titleId = `v2-inventory-category-${safeId(category.id, "other")}`;
    return `<section class="v2-inventory-category" data-v2-inventory-category="${escapeHTML(category.id)}" aria-labelledby="${titleId}">
      ...
      <ul class="v2-inventory-grid" role="list">${items.map(renderItem).join("")}</ul>
    </section>`;
  }
  ...
  function render(rawContext) {
    ...
    const access = context.access.available
      ? ""
      : `<section class="v2-inventory-access" role="status"><strong>${escapeHTML(context.access.label)}</strong><p>${escapeHTML(context.access.reason)}</p></section>`;
    const content = context.items.length
      ...
      : `<section class="v2-inventory-empty" role="status"><h3>${escapeHTML(context.emptyTitle)}</h3><p>${escapeHTML(context.emptyMessage)}</p></section>`;
    ...
    return `<section class="v2-inventory-view" aria-labelledby="v2-inventory-title">
      ...
      ${access}<div class="v2-inventory-content">${content}</div>
    </section>`;
  }
