(function attachBoxeurOnboarding(root, factory) {
  ...
  const KIND = "boxeur-deux-onboarding";
  const LEGACY_KINDS = Object.freeze(["boxeur-deux-v2-onboarding"]);
  ...
  function isOnboardingState(value) {
    return Boolean(value && typeof value === "object"
      && (value.kind === KIND || LEGACY_KINDS.includes(value.kind))
  ...
  function isDeveloperSource(source, options) {
    ...
    return options.developerMode === true
      ...
      || career.careerDeveloperTest?.active === true;
  }
  ...
  function normalizeState(source = {}, options = {}) {
    ...
    const jobId = canonical
      ...
      : cleanId(raw.jobId || raw.careerJob?.id);
