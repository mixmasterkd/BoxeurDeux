(function attachBoxeurWeek(root, factory) {
  ...
  const trainingApi = isCommonJs ? require("./v2-training-engine.js") : root && root.BoxeurTraining;
  const recoveryApi = isCommonJs ? require("./v2-recovery-engine.js") : root && root.BoxeurRecovery;
  ...
  function normalizeWorkActivity(work = {}, shift = {}, index = 0) {
    ...
    return BoxeurTime.normalizeActivity({
      id: String(shift.activityId || work.activityId || `v2-work:${work.id || "job"}`),
  ...
  function makeRecord(before, after, beforeMoney, afterMoney, primitive, detail = {}) {
    ...
    return {
      ...
      xpAward: finiteNumber(detail.xpAward, 0, 0, Number.MAX_SAFE_INTEGER),
  ...
  function aggregateSummary(initial, current, mode, plan, records, usedBefore, status, stop, warnings) {
    ...
    return {
      ...
      xpAward: roundTo(records.reduce((sum, record) => sum + record.xpAward, 0)),
