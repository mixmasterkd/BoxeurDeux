(function attachBoxeurCareerMigration(root, factory) {
  ...
  if (root) root.BoxeurCareerMigration = api;
  ...
  if (!BoxeurTime || typeof BoxeurTime.createState !== "function") {
    throw new Error("BoxeurTime doit être chargé avant career-migration.js.");
  }

  const CAPSULE_KIND = "boxeur-deux-career-capsule";
  const CAPSULE_VERSION = 3;
  const LEGACY_CAPSULE_KINDS = Object.freeze(["boxeur-deux-career-v2-capsule"]);
  const LEGACY_CAPSULE_VERSIONS = Object.freeze([2]);
  ...
  const PERSISTED_KEY_ALIASES = Object.freeze({
    v2PreviewCapsule: "careerCapsule",
    v2SupplementState: "supplementState",
    v2TrainerState: "trainerState",
    v2WeekPlannerState: "weekPlannerState",
  });

  const PERSISTED_STRING_ALIASES = Object.freeze({
    "boxeur-deux-career-v2-capsule": CAPSULE_KIND,
    "boxeur-deux-v2-onboarding": "boxeur-deux-onboarding",
    "boxeur-v2-progression": "boxeur-progression",
    "boxeur-v2-supplements": "boxeur-supplements",
    "boxeur-v2-private-trainer": "boxeur-private-trainer",
    "boxeur-v2-week-planner": "boxeur-week-planner",
    "boxeur-v2-week-plan-commit": "boxeur-week-plan-commit",
    "v2-progression": "progression",
    "v2-sparring-technique": "sparring-technique",
    "v2-remy-sparring": "remy-sparring",
    "v2-practice-sparring": "practice-sparring",
  });

  const PERSISTED_STRING_PREFIX_ALIASES = Object.freeze([
    ["v2-work:", "work:"],
    ["v2-planner:", "planner:"],
    ["v2-remy-sparring:", "remy-sparring:"],
    ["v2-practice-sparring:", "practice-sparring:"],
  ...
  function migratePersistedString(value) {
    if (Object.prototype.hasOwnProperty.call(PERSISTED_STRING_ALIASES, value)) {
      return PERSISTED_STRING_ALIASES[value];
    }
    const prefix = PERSISTED_STRING_PREFIX_ALIASES.find(([legacy]) => value.startsWith(legacy));
    return prefix ? `${prefix[1]}${value.slice(prefix[0].length)}` : value;
  }

  function migratePersistedValue(value, seen = new Map()) {
    if (typeof value === "string") return migratePersistedString(value);
    if (value === null || typeof value !== "object") return value;
    if (seen.has(value)) return seen.get(value);
    if (value instanceof Date) return new Date(value.getTime());
    if (Array.isArray(value)) {
      const result = [];
      seen.set(value, result);
      value.forEach(item => result.push(migratePersistedValue(item, seen)));
      return result;
    }
    const result = {};
    seen.set(value, result);
    const keys = Object.keys(value);
    keys.filter(key => !PERSISTED_KEY_ALIASES[key]).forEach(key => {
      result[key] = migratePersistedValue(value[key], seen);
    });
    keys.filter(key => PERSISTED_KEY_ALIASES[key]).forEach(key => {
      const currentKey = PERSISTED_KEY_ALIASES[key];
      if (!Object.prototype.hasOwnProperty.call(result, currentKey)) {
        result[currentKey] = migratePersistedValue(value[key], seen);
      }
    });
    return result;
  }

  function isRecognizedCapsule(value) {
    if (!value || typeof value !== "object" || !value.timeState) return false;
    const current = value.kind === CAPSULE_KIND && [CAPSULE_VERSION, ...LEGACY_CAPSULE_VERSIONS].includes(Number(value.version));
    const legacy = LEGACY_CAPSULE_KINDS.includes(value.kind) && LEGACY_CAPSULE_VERSIONS.includes(Number(value.version));
    return Boolean((current || legacy) && Object.prototype.hasOwnProperty.call(value, "legacySnapshot"));
  }

  function migrateCareerSnapshot(snapshot, options = {}) {
    const result = migratePersistedValue(snapshot);
    if (!result || typeof result !== "object") return result;
    if (result.state && typeof result.state === "object") {
      result.version = boundedInteger(options.targetVersion, EXPECTED_SOURCE_VERSION, 1, 999);
      if (result.careerCapsule && isRecognizedCapsule(result.careerCapsule)) {
        result.careerCapsule = upgradeCareerCapsule(result.careerCapsule, options);
      }
    } else if (result.profile && typeof result.profile === "object") {
      return {
        version: boundedInteger(options.targetVersion, EXPECTED_SOURCE_VERSION, 1, 999),
        savedAt: typeof result.savedAt === "string" ? result.savedAt : new Date(0).toISOString(),
        state: result,
        weeklyPlan: Array.isArray(result.weeklyPlan) ? migratePersistedValue(result.weeklyPlan) : [],
      };
    }
    return result;
  }

  function upgradeCareerCapsule(capsule, options = {}) {
    if (!isRecognizedCapsule(capsule)) throw new TypeError("Une capsule de carrière reconnue est requise.");
    const upgraded = migratePersistedValue(capsule);
    upgraded.kind = CAPSULE_KIND;
    upgraded.version = CAPSULE_VERSION;
    upgraded.legacySnapshot = migrateCareerSnapshot(upgraded.legacySnapshot, options);
    return upgraded;
  }
  ...
  function isCareerCapsule(value) {
    return isRecognizedCapsule(value);
  }
   ...
   */
  function migrateToCareerCapsule(snapshot, options = {}) {
    if (isCareerCapsule(snapshot)) {
      const upgraded = ensureLegacyTrainingCarry(upgradeCareerCapsule(snapshot, options));
      upgraded.timeState = BoxeurTime.upgradeState(upgraded.timeState);
      return upgraded;
    }

    const legacySnapshot = migrateCareerSnapshot(snapshot, options);
    const career = stateFromSnapshot(legacySnapshot);
  ...
  function syncTimeStateToCareer(snapshotOrCapsule, suppliedTimeState) {
    const capsule = isCareerCapsule(snapshotOrCapsule) ? snapshotOrCapsule : null;
  ...
  /** Returns an independent copy of the migrated career snapshot. */
  function rollbackCareerMigration(capsule) {
    if (!isCareerCapsule(capsule)) throw new TypeError("Une capsule de migration de carrière valide est requise.");
  ...
  return Object.freeze({
    CAPSULE_KIND,
    CAPSULE_VERSION,
    LEGACY_CAPSULE_KINDS,
    LEGACY_CAPSULE_VERSIONS,
    LEGACY_TRAINING_CARRY_VERSION,
    EXPECTED_SOURCE_VERSION,
    STAT_KEYS,
    PERSISTED_KEY_ALIASES,
    PERSISTED_STRING_ALIASES,
    isCareerCapsule,
    migratePersistedValue,
    migrateCareerSnapshot,
    inferCareerPhase,
    progressToResidualStimulus,
    migrateToCareerCapsule,
    syncTimeStateToCareer,
    rollbackCareerMigration,
  });
});
