(function attachBoxeurWeekPlanner(root, factory) {
  ...
  const STATE_KIND = "boxeur-week-planner";
  const LEGACY_STATE_KINDS = Object.freeze(["boxeur-v2-week-planner"]);
  ...
  function migratePlannerState(source) {
    if (!source || typeof source !== "object" || (source.kind !== STATE_KIND && !LEGACY_STATE_KINDS.includes(source.kind))) return source;
    const next = clone(source);
    next.kind = STATE_KIND;
    if (source.schemaVersion === SCHEMA_VERSION) return next;
