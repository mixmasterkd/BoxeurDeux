(function attachBoxeurStrengthView(root, factory) {
  ...
  function render(rawContext) {
    ...
    return `<div class="v2-strength-view" data-v2-strength-access="${escapeHTML(context.access.state)}" data-career-status="${escapeHTML(context.careerStatus)}">
      ...
      <section class="v2-strength-energy" aria-labelledby="v2-strength-week-energy-title"><div><p class="eyebrow">Programme hebdomadaire</p><h3 id="v2-strength-week-energy-title">Énergie restante de la semaine</h3></div><div class="v2-strength-energy-meter"><progress max="${context.weekCapacity.total}" value="${context.weekCapacity.remaining}" aria-label="Énergie hebdomadaire restante : ${context.weekCapacity.remaining} sur ${context.weekCapacity.total}">${context.weekCapacity.remaining}/${context.weekCapacity.total}</progress><strong>${context.weekCapacity.remaining}/${context.weekCapacity.total}</strong></div><div><span>Déjà réservée</span><strong>${context.weekCapacity.used}</strong></div></section>
