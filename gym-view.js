(function attachBoxeurGymView(root, factory) {
  ...
  function renderSparringCard(context) {
    ...
    const disabled = sparring.available ? "" : " disabled aria-disabled=\"true\" aria-describedby=\"career-gym-sparring-reason\"";
    const buttonLabel = context.sparring.immediate
      ...
      : sparring.available
        ...
        : "Sparring verrouillé";
    return `<article class="career-gym-sparring-card career-gym-action-card ${sparring.available ? "available" : "locked"}" id="career-gym-sparring-card" data-career-sparring-state="${sparring.status}" aria-labelledby="career-gym-sparring-title" tabindex="-1">
      <p class="eyebrow">Activité distincte</p>
      <h3 id="career-gym-sparring-title">Sparring</h3>
      <p>Une opposition interactive avec un partenaire. Le sparring ne fait jamais partie d’une séance personnalisée.</p>
      <div class="career-gym-sparring-state" id="career-gym-sparring-reason" role="note"><span aria-hidden="true">${sparring.available ? "✓" : "🔒"}</span><div><strong>${escapeHTML(sparring.label)}</strong><p>${escapeHTML(sparring.detail)}</p></div></div>
      <button type="button" class="${sparring.available ? "primary-button" : "secondary-button"}" data-career-sparring-activity="cta"${disabled}>${buttonLabel}</button>
    </article>`;
  }
  ...
  function renderCoachCard(context) {
    ...
    const notice = context.coach.notice ? `<p class="career-gym-coach-notice">${escapeHTML(context.coach.notice)}</p>` : "";
    return `<article class="career-gym-coach-card career-gym-action-card recommended" aria-labelledby="career-gym-coach-title">
      <div class="career-gym-action-heading"><span>Recommandé</span><small>${context.coach.durationMinutes} min</small></div>
      <p class="eyebrow">Préparée par ${escapeHTML(context.coach.name)}</p>
      <h3 id="career-gym-coach-title">${escapeHTML(context.coach.sessionTitle)}</h3>
      <p>${escapeHTML(context.coach.sessionSummary)}</p>
      ${notice}
      <button type="button" class="${buttonClass}" data-career-coach-session aria-pressed="${context.coach.planned}"${disabled}>${buttonLabel}</button>
    </article>`;
  }
  ...
  function renderMenu(menuId, rawContext) {
    ...
    if (id === "ring") {
      ...
      return `<section class="career-gym-menu" aria-labelledby="career-gym-menu-title">
        <header><div><p class="eyebrow">GYM de boxe</p><h2 id="career-gym-menu-title">${title}</h2></div><button type="button" class="secondary-button" data-career-gym-menu-close>Retour au GYM</button></header>
        <p>Le sparring est une activité distincte : il ne fait jamais partie d’une séance personnalisée.</p>
        <div class="career-gym-menu-actions">${renderSparringCard(context)}</div>
      </section>`;
    }
    return "";
  }
  ...
  function render(rawContext) {
    ...
    const zones = ZONES.map(zone => {
      ...
      const reason = sparringBlocked
        ...
        : "";
      const lock = membershipBlocked || sparringBlocked || recreationalBlocked ? `<span class="career-gym-hotspot-lock" aria-hidden="true">🔒</span>` : "";
      const reasonMarkup = reason ? `<span class="sr-only">${escapeHTML(reason)}</span>` : "";
      return `<button type="button" class="career-gym-hotspot career-gym-hotspot-${zone.id}" data-career-gym-zone="${zone.id}" aria-label="${escapeHTML(display.label)}. ${escapeHTML(display.detail)}${escapeHTML(reason)}"${disabled}><strong>${escapeHTML(display.label)}</strong><small>${escapeHTML(display.detail)}</small>${lock}${reasonMarkup}</button>`;
    }).join("");
    const membershipLock = context.membership.active ? "" : `<div class="career-gym-access-lock" id="career-gym-membership-lock-reason" role="note">
    ...
    return `<div class="career-gym-view career-place-view" data-career-status="${context.careerStatus}" data-membership-active="${context.membership.active}">
      <header class="career-gym-header career-place-header">
        <div><p class="eyebrow">GYM de boxe</p><h2>Bienvenue au GYM, ${escapeHTML(context.profile.firstName)}</h2><p class="career-place-meta">${escapeHTML(context.careerStatusLabel)} · Semaine ${context.clock.week} · ${escapeHTML(context.clock.dayLabel)} · ${escapeHTML(context.clock.dateLabel)}</p></div>
        <button type="button" class="secondary-button" data-career-leave-gym>Retour à la carte</button>
      </header>
      <div class="career-gym-layout career-place-layout">
        <section class="career-gym-floor career-place-scene" aria-labelledby="career-gym-floor-title">
          <h3 id="career-gym-floor-title" class="sr-only">Zones interactives du GYM</h3>
          <picture>
          ...
          <div class="career-gym-hotspots">${zones}</div>
          ${membershipLock}
        </section>
        <aside class="career-gym-dashboard career-place-dashboard" aria-label="Séance et état actuel">
          ${renderWeekPlan(context)}
          <section class="career-gym-readiness career-place-condition ${context.condition.preparationTone}">
          ...
          <section class="career-gym-membership career-place-card ${context.membership.active ? "active" : "inactive"}">
            ...
            <p class="career-gym-membership-hint">Utilise le bouton <strong>Accueil</strong> dans le GYM pour ${context.membership.active ? "gérer l’accès" : "t’inscrire"}.</p>
          </section>
        </aside>
      </div>
    </div>`;
  ...
  function renderComposer(rawContext) {
    ...
    const presets = PRESETS.map(preset => {
      ...
      return `<button type="button" class="career-session-preset${active ? " selected" : ""}" data-career-session-preset="${preset.id}" aria-pressed="${active}"${membershipLocked || context.condition.trainingBlocked ? " disabled aria-disabled=\"true\"" : ""}><strong>${escapeHTML(preset.label)}</strong><small>${escapeHTML(preset.detail)}</small></button>`;
    }).join("");
    const exerciseButtons = EXERCISES.map(exercise => {
      ...
      return `<button type="button" class="career-exercise-choice${isSelected ? " selected" : ""}" data-career-exercise="${exercise.id}" aria-pressed="${isSelected}" aria-label="${escapeHTML(`${exercise.label}. ${exercise.roleLabel}. ${stateLabel}`)}"${disabled}><span class="career-exercise-heading-row"><strong>${escapeHTML(exercise.label)}</strong><b>${exercise.energyCost > 0 ? `−${exercise.energyCost} E` : `+${Math.abs(exercise.fatigueGain)} récup.`}</b></span><small><span>${escapeHTML(exercise.roleLabel)}</span> · ${escapeHTML(exercise.gains)}</small><small>${escapeHTML(exercise.detail)}</small><em>${exercise.durationMinutes} min · ${escapeHTML(stateLabel)}</em></button>`;
    }).join("");
    const blocks = selected.length
      ? selected.map((id, index) => {
          const exercise = EXERCISES.find(item => item.id === id);
          return `<li><span>Activité ${index + 1}</span><strong>${escapeHTML(exercise.label)}</strong><small>−${exercise.energyCost} E · ${exercise.durationMinutes} min</small><button type="button" data-career-exercise="${exercise.id}" aria-label="Retirer ${escapeHTML(exercise.label)}">Retirer</button></li>`;
        }).join("")
      : `<li class="empty">Choisis ta première activité ci-dessous. Tu peux commencer petit ou bâtir une séance plus complète.</li>`;

    return `<section class="career-session-composer" aria-labelledby="career-composer-title">
      <header><div><p class="eyebrow">Séance personnalisée</p><h2 id="career-composer-title">Bâtis ta séance avec ton énergie</h2></div><button type="button" data-career-close-composer aria-label="Fermer le compositeur">Fermer</button></header>
      ...
      ${membershipLocked ? `<p class="career-composer-membership-note" role="status"><strong>Inscription requise.</strong> Retourne à la réception pour débloquer les activités du GYM.</p>` : ""}
      <section class="career-composer-energy" aria-label="Capacité disponible pendant la composition" aria-live="polite"><div><span>Capacité restante de la semaine</span><strong>${projectedWeekEnergy} / ${context.weekCapacity.total}</strong></div><progress max="${context.weekCapacity.total}" value="${projectedWeekEnergy}">${projectedWeekEnergy}/${context.weekCapacity.total}</progress><p>${context.weekCapacity.remaining} disponible avant cette séance · coût estimé ${context.draftWeekCost}</p><div><span>Énergie physique après la séance</span><strong>${projectedEnergy} %</strong></div></section>
      <section class="career-session-presets" aria-labelledby="career-session-presets-title"><div><p class="eyebrow">Modèles rapides</p><h3 id="career-session-presets-title">Choisir une spécialisation</h3></div><div>${presets}</div></section>
      <div class="career-composer-state" aria-live="polite"><strong>${selected.length} activité${selected.length > 1 ? "s" : ""}</strong><span>Durée prévue : ${context.draftDurationMinutes} min</span><span>Énergie après : ${projectedEnergy} %</span><span>Fatigue après : ${projectedFatigue} %</span></div>
      <ul class="career-session-structure" aria-label="Structure minimale de la séance"><li class="${hasPreparation ? "complete" : "missing"}"><span aria-hidden="true">${hasPreparation ? "✓" : "○"}</span> Préparation</li><li class="${hasWork ? "complete" : "missing"}"><span aria-hidden="true">${hasWork ? "✓" : "○"}</span> Travail principal</li><li class="${hasCooldown ? "complete" : "missing"}"><span aria-hidden="true">${hasCooldown ? "✓" : "○"}</span> Retour au calme</li></ul>
      <ol class="career-session-blocks" aria-label="Activités choisies">${blocks}</ol>
      <div class="career-exercise-grid" aria-label="Exercices disponibles">${exerciseButtons}</div>
      <footer><button type="button" class="secondary-button" data-career-close-composer>Annuler</button><button type="button" class="primary-button" data-career-confirm-session${structureReady && !context.condition.trainingBlocked && !membershipLocked ? "" : " disabled aria-disabled=\"true\""}>Ajouter à ma semaine</button></footer>
    </section>`;
  }
  ...
  function renderResult(rawResult) {
    ...
    const highlightMarkup = highlights.length
      ? `<ul class="career-session-highlights">${highlights.map(item => `<li>${escapeHTML(item)}</li>`).join("")}</ul>`
      : "";
    const nextStep = result.nextStep ? `<p class="career-session-next-step"><strong>Conseil :</strong> ${escapeHTML(result.nextStep)}</p>` : "";

    return `<section class="career-session-result" aria-labelledby="career-session-result-title" aria-live="polite">
      <p class="eyebrow">Bilan du GYM</p><h2 id="career-session-result-title">${escapeHTML(title)}</h2>
      <p>${escapeHTML(summary)}</p><p class="career-session-result-duration">Durée : <strong>${durationMinutes} min</strong></p>
      <ul class="career-session-result-changes" aria-label="Effets de la séance">${changeMarkup}</ul>
      ${highlightMarkup}${nextStep}
      <div class="career-session-result-actions"><button type="button" class="secondary-button" data-career-result-close>Rester au GYM</button><button type="button" class="primary-button" data-career-leave-gym>Retour à la carte</button></div>
    </section>`;
  }
