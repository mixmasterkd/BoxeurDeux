(function attachBoxeurWeekView(root, factory) {
  ...
  function capacityMarkup(context, compact = false) {
    ...
    return `<section class="career-week-capacity ${capacity.zone}" aria-labelledby="career-week-capacity-title">
      <div class="career-week-capacity-heading"><span id="career-week-capacity-title">Capacité restante de la semaine</span><strong>${capacity.remaining}/${capacity.total}</strong></div>
      ...
      <div class="career-week-capacity-meta"><b>${escapeHTML(capacity.zoneLabel)}</b><span>${capacity.spent} capacité réservée</span></div>
      ${compact ? "" : `<p>${escapeHTML(capacity.detail)}</p>`}
    </section>`;
  }

  function compactPlanMarkup(context) {
    if (!context.plan.items.length) return `<p class="career-week-empty">Aucune activité facultative n’est encore planifiée.</p>`;
    const visible = context.plan.items.slice(0, 4);
    const extra = context.plan.items.length - visible.length;
    return `<ul class="career-week-compact-items" aria-label="Activités déjà planifiées">${visible.map(item => `<li class="${item.tone}"><span>${escapeHTML(item.label)}</span><b>${item.cost > 0 ? `−${item.cost}` : item.cost < 0 ? `+${Math.abs(item.cost)}` : "Prévu"}</b></li>`).join("")}</ul>${extra > 0 ? `<p class="career-week-more">+${extra} autre${extra > 1 ? "s" : ""} activité${extra > 1 ? "s" : ""}</p>` : ""}`;
  }
  ...
  function renderLauncher(rawContext) {
    ...
    return `<section class="career-week-launcher" data-career-week-zone="${context.capacity.zone}" aria-labelledby="career-week-launcher-title">
      <div class="career-week-launcher-heading"><div><p class="eyebrow">Semaine ${context.week} · plan modifiable</p><h3 id="career-week-launcher-title">Bâtis ta semaine</h3></div><span>${context.plan.items.length} choix</span></div>
      ${capacityMarkup(context, true)}
      ${compactPlanMarkup(context)}
      <div class="career-week-launcher-actions">
        <button type="button" class="secondary-button" data-career-week-quick${quickDisabled}>${escapeHTML(context.quick.label)}</button>
        <button type="button" class="primary-button" data-career-week-detailed>Confirmer semaine</button>
      </div>
      ${reason ? `<p class="career-week-blocker" role="status">${escapeHTML(reason)}</p>` : ""}
    </section>`;
  }
  ...
  function planItemMarkup(item) {
    ...
    const action = item.removable
      ? `<button type="button" data-career-week-remove="${escapeHTML(item.id)}" aria-label="Retirer ${escapeHTML(item.label)} du plan">Retirer</button>`
      : `<span class="career-week-item-fixed">Prévu par défaut</span>`;
  ...
  function renderPlan(rawContext) {
    ...
    const items = context.plan.items.length
      ...
      : `<li class="neutral career-week-plan-empty"><div><strong>Ton plan est vide</strong><small>Retourne à la carte et visite un lieu pour ajouter une activité.</small></div></li>`;
    const confirmDisabled = context.confirm.available ? "" : ' disabled aria-disabled="true"';
    return `<section class="career-week-plan" aria-labelledby="career-week-plan-title">
      <header><div><p class="eyebrow">Semaine ${context.week} · tout reste modifiable</p><h2 id="career-week-plan-title">${escapeHTML(context.plan.title)}</h2></div><button type="button" data-career-week-plan-close aria-label="Fermer le plan de la semaine">Fermer</button></header>
      <p>${escapeHTML(context.plan.summary)}</p>
      ${capacityMarkup(context)}
      ${context.risk ? `<aside class="career-week-risk ${context.risk.tone}" role="note"><span aria-hidden="true">!</span><div><strong>${escapeHTML(context.risk.title)}</strong><p>${escapeHTML(context.risk.detail)}</p></div></aside>` : ""}
      <ul class="career-week-plan-items" aria-label="Contenu du programme">${items}</ul>
      <p class="career-week-engine-note">Les activités ne sont pas encore accomplies. Elles seront résolues seulement lorsque tu confirmeras la semaine.</p>
      ${context.confirm.reason ? `<p class="career-week-blocker" role="status">${escapeHTML(context.confirm.reason)}</p>` : ""}
      <footer><button type="button" class="secondary-button" data-career-week-plan-close>Continuer à planifier</button><button type="button" class="primary-button" data-career-week-confirm${confirmDisabled}>${escapeHTML(context.confirm.label)}</button></footer>
    </section>`;
  }
  ...
  function renderSummary(rawSummary) {
    ...
    const eventsMarkup = events.length
      ? `<section class="career-week-summary-events" aria-labelledby="career-week-alerts-title"><h3 id="career-week-alerts-title">À retenir</h3><ul>${events.map(item => `<li class="${item.tone}"><strong>${escapeHTML(item.label)}</strong><span>${escapeHTML(item.detail)}</span></li>`).join("")}</ul></section>`
      : "";
    ...
    const guide = raw.guide && typeof raw.guide === "object"
      ? `<section class="career-week-summary-guide" aria-labelledby="career-week-summary-guide-title"><p class="eyebrow">Guide récréatif</p><h3 id="career-week-summary-guide-title">${escapeHTML(raw.guide.title || "Comprendre le bilan")}</h3><p>${escapeHTML(raw.guide.detail || "Ce bilan résume les effets de la semaine.")}</p>${raw.guide.next ? `<strong>${escapeHTML(raw.guide.next)}</strong>` : ""}</section>`
      : "";
    return `<section class="career-week-summary" aria-labelledby="career-week-summary-title" aria-live="polite">
      <p class="eyebrow">Semaine ${weekFrom} terminée</p><h2 id="career-week-summary-title">${escapeHTML(raw.title || `Bienvenue à la semaine ${weekTo}`)}</h2>
      <p>${escapeHTML(raw.summary || "Ton plan, ton emploi et ta récupération ont été résolus.")}</p>
      ${guide}
      <ul class="career-week-summary-changes" aria-label="Changements de la semaine">${changesMarkup}</ul>
      ${eventsMarkup}
      <div class="career-week-summary-actions"><button type="button" class="primary-button" data-career-week-summary-close>${escapeHTML(raw.actionLabel || "Retour à la carte")}</button></div>
    </section>`;
  }
