(function attachBoxeurFighterView(root, factory) {
  ...
  const EXTENSION_SLOTS = Object.freeze({
    manualLevelUp: "[data-career-level-up-slot]",
  });
  ...
  function renderStat(key, context) {
    ...
    return `<article class="career-fighter-stat" aria-labelledby="career-fighter-stat-${key}">
      <div><span id="career-fighter-stat-${key}">${label}</span><strong>${value}</strong></div>
      <div class="career-fighter-progress" role="progressbar" aria-label="XP ${label.toLocaleLowerCase("fr-CA")} : ${xp.total} sur ${xp.nextThreshold}" aria-valuemin="0" aria-valuemax="${xp.nextThreshold}" aria-valuenow="${xp.total}">
        <span style="width:${progress}%"></span>
      </div>
      <div class="career-fighter-stat-meta"><small><strong>${xp.total} / ${xp.nextThreshold} XP</strong></small><small class="${xp.pendingXp > 0 ? "pending" : "clear"}">${pendingLabel}</small></div>
    </article>`;
  }

  function renderLevelProgress(context) {
    const progress = context.levelProgress.progress;
    return `<div class="career-fighter-level-progress">
      <div><span>XP générale</span><strong>${context.experience} / ${context.levelProgress.nextFloor} XP</strong></div>
      <div class="career-fighter-progress" role="progressbar" aria-label="XP générale : ${context.experience} sur ${context.levelProgress.nextFloor} avant le niveau ${context.level + 1}" aria-valuemin="0" aria-valuemax="${context.levelProgress.nextFloor}" aria-valuenow="${context.experience}">
        <span style="width:${progress}%"></span>
      </div>
    </div>`;
  }
  ...
  function renderMedalTotals(medals, compact = false) {
    ...
    return `<span class="career-fighter-medal-totals${compact ? " compact" : ""}" aria-label="${medals.totals.gold} or, ${medals.totals.silver} argent et ${medals.totals.bronze} bronze">${order.map(item => `<span data-medal="${item.id}"><i aria-hidden="true"></i><strong>${item.label}</strong> ${medals.totals[item.id]}</span>`).join("")}</span>`;
  }
  ...
  function renderMedalPalmares(context) {
    ...
    const content = medals.count
      ? `<div class="career-fighter-medal-overview">${renderMedalTotals(medals)}</div><div class="career-fighter-medal-list">${medals.rows.map(row => `<article class="career-fighter-medal-row"><strong>${escapeHTML(row.label)}</strong><span><b data-medal="gold">Or ${row.gold}</b><b data-medal="silver">Argent ${row.silver}</b><b data-medal="bronze">Bronze ${row.bronze}</b></span></article>`).join("")}</div>`
      : `<p class="career-fighter-empty">Aucune médaille pour le moment.<br><small>Une victoire en finale donne l’or; une défaite en finale donne l’argent et une défaite en demi-finale donne le bronze.</small></p>`;
    return `<section class="career-fighter-medals" aria-labelledby="career-fighter-medals-title">
      <div class="career-fighter-section-heading"><div><p class="eyebrow">Bilan de tournoi</p><h3 id="career-fighter-medals-title">Médailles</h3></div><p>Les podiums de tous les tournois amateurs, y compris les compétitions indépendantes, sont conservés ici.</p></div>
      ${content}
    </section>`;
  }
  ...
  function render(rawContext) {
    ...
    return `<section class="career-fighter-view" aria-labelledby="career-fighter-title">
      <header class="career-fighter-header">
        <div><p class="eyebrow">Fiche du boxeur</p><h2 id="career-fighter-title">${escapeHTML(context.profile.firstName)}${nickname} ${escapeHTML(context.profile.lastName)}</h2></div>
      </header>
      <div class="career-fighter-layout">
        <aside class="career-fighter-identity">
          <div class="career-fighter-visual">
            <div class="career-fighter-portrait portrait-crop" style="--portrait-index:${context.profile.portraitId}" role="img" aria-label="Portrait de ${escapeHTML(context.profile.firstName)}">
              <img src="${portraitAsset(context.profile.sex)}" width="1152" height="768" alt="" />
            </div>
            ${renderLevelProgress(context)}
            <div class="career-fighter-badges"><span>${escapeHTML(context.statusLabel)} – Niveau ${context.level}</span></div>
          </div>
          <dl>
            ...
            <div><dt>Médailles</dt><dd class="career-fighter-medal-summary">${compactMedals}</dd></div>
        ...
        <div class="career-fighter-details">
          <section aria-labelledby="career-fighter-progression-title">
            <div class="career-fighter-section-heading"><div><p class="eyebrow">XP ciblée cumulative</p><h3 id="career-fighter-progression-title">Tes quatre qualités de boxe</h3></div><p>L’entraînement crée de l’XP ciblée. Les nuits l’assimilent; atteindre le prochain seuil donne +1 à la qualité sans perdre l’excédent.</p></div>
            <div class="career-fighter-stats">${STAT_KEYS.map(key => renderStat(key, context)).join("")}</div>
            <div data-career-level-up-slot data-level-points="${context.levelProgress.manualPoints}" hidden></div>
          </section>
          ...
          <section class="career-fighter-save" aria-labelledby="career-fighter-save-title">
            <div><p class="eyebrow">Sauvegarde externe</p><h3 id="career-fighter-save-title">Conserver une copie de ta carrière</h3><p>Le fichier contient ta progression complète et pourra être réimporté depuis l’écran d’accueil.</p></div>
            <button type="button" class="primary-button" data-career-export-career>Télécharger la sauvegarde JSON</button>
          </section>
        </div>
      </div>
    </section>`;
